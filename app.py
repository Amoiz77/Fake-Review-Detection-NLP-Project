...
def check_mid_sentence(text):
    """Friend's Fix #4: Ignore trailing quotes/brackets and digits so valid

    sentence endings aren't flagged as cut-off mid-sentence.
    """
    t = str(text).strip()
    if not t:
        return 0
...
def extract_features(df):
    ...
    # Synthetic / Repeated Phrase Indicators
    repeated_phrases = [
...
# Friend's Fix #1: CG (fake) = 0, OR (genuine) = 1
...
# Increased max_features to capture more subtle vocabulary patterns
...
print(
    "✅ Model training complete. Artifacts saved: model.pkl, tfidf_word.pkl, tfidf_char.pkl"
)
